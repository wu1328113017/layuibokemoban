...
var articles = {
	...
	dg1: function(data, index, length) {
		...
		if (data[index] != null) {
			...
			if(data[index].content && data[index].content.length>100){
				data[index].content = data[index].content.substring(0,100) + "...";
			}
		}
		index++;
...
myapp.myAjax1({
	...
	if(result.length > 5){
		for(var i=0;i<=4;i++){
			a.$data.category_list.push(result[i]);
		}
		var more = [];
		...
		a.$data.category_list.push(e);
		a.$data.category_list_more = more;
	}else{
